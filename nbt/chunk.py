...
class Chunk(object):
	...
	def get_map(self):
		...
		block_colors = {
			...
			31: {'h':94, 's':42, 'l':32},   # Tall Grass
			32: {'h':27, 's':51, 'l':15},   # Dead Shrub
...
class BlockArray(object):
	...
	def generate_contour(self, as_array=False):
		bytes = []
		invalid_blocks = [0, 6, 8, 9, 10, 11, 17, 18, 26, 30, 31, 32, 37, 38, 39, 40, 50, 51, 52, 55, 59, 63, 65, 66, 68, 69, 70, 71, 72, 75, 76, 77, 78, 79, 81, 83, 85, 86, 91, 92, 93, 94]
		for z in range(16):
			for x in range(16):
				for y in range(127,-1,-1):
					offset = y + z*128 + x*128*16
